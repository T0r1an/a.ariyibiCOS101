...
fn main() {
    ...
    let a:f32 = input1.trim().parse().expect("Not a valid member");

    println!("Enter second edge of the triangle: ");
    io::stdin().read_line(&mut input2).expect("Naot a valid string");
    let b:f32 = input2.trim().parse().expect("Not a valid member");

    println!("Enter third edge of triangle: ");
    io::stdin().read_line(&mut input3).expect("Not a valid string");
    let c:f32 = input3.trim().parse().expect("Not a valid member");
